### bias correction + aggregation + benchmark comparison for I*M product (Sicily_Rainfall_Reconstruction_I3) ###
...
MODEL_SAMPLE_FILE      = r'outputs/reconstruction_I3/aggregated_monthly_totals/monthly_rainfall_1951-01.nc'
MODEL_AGG_FOLDER       = r'outputs/reconstruction_I3/aggregated_monthly_totals'
MAGNITUDE_FOLDER       = r'outputs/reconstruction_I3/Phase_II/IM_Product_Grids'
OUTPUT_CORRECTED_FOLDER= r'outputs/reconstruction_I3/Phase_II/BiasAdjusted Maps'
CORRECTOR_FILE         = r'outputs/reconstruction_I3/monthly_corrector_maps_1951-2022.nc'
BENCHMARK_EXCEL        = r'data/input/monthly_rainfall.xlsx'
BASE_PATH              = Path(r'outputs/reconstruction_I3')
MAGNITUDE_VAR          = 'IM_product'
MAG_FILE_PATTERN       = 'im_product_*.nc'
CORRECTED_FILE_PATTERN = 'im_product_*.nc'
...
# ---------- 3. apply corrector to daily IM_product maps ----------
corrector_ds = xr.open_dataset(CORRECTOR_FILE)
for date in tqdm(valid_months, desc='Correcting daily maps'):
    y,m = date.year, date.month
    mag_file = os.path.join(MAGNITUDE_FOLDER, f'im_product_{y:04d}-{m:02d}.nc')
    ...
    with xr.open_dataset(mag_file) as ds_mag:
        ...
        out = os.path.join(OUTPUT_CORRECTED_FOLDER, f'im_product_{y:04d}-{m:02d}.nc')
...
class MonthlyAggregator:
    ...
    def get_available_months(self):
        files = list(self.magnitude_path.glob('im_product_*.nc'))
    ...
    def aggregate_month(self, month_str):
        ...
        ds = xr.open_dataset(self.magnitude_path / f'im_product_{month_str}.nc')
