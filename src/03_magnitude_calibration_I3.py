### code for Phase II: rain magnitude modeling, method I3-IIA ###
...
def run_workflow_matern_enhanced(daily_df, final_group_days, occurrence, dem=None, workflow_type="OK"):
    ...
    save_path = f'outputs/phase2_results/{workflow_type}_enhanced12/'
    ...
    for category, days in final_group_days.items():
        ...
        try:
            ...
            for day in pbar_vario:
                sub = cat_df_transformed[(cat_df_transformed['day'] == day) ]
                if len(sub) < 2:
                    continue
...
def create_IM_product_from_file(workflow_type: str, occurrence_ds: xr.Dataset, 
    ...
    try:
        magnitude_ds = xr.open_dataset(magnitude_file)
        magnitude_times = magnitude_ds.time.values
        occurrence_times = occurrence_ds.time.values
        common_times = np.intersect1d(magnitude_times, occurrence_times)
        if len(common_times) == 0:
            warnings.warn("No common time steps between magnitude and occurrence datasets.")
            return
        print(f"Found {len(common_times)} common time steps")
        magnitude_common = magnitude_ds.sel(time=common_times)
        occurrence_common = occurrence_ds.sel(time=common_times)
        IM_product = occurrence_common['prob_map'] * magnitude_common['rainfall_magnitude']
        IM_ds = xr.Dataset(
            {
                "IM_product": (("time", "y", "x"), IM_product.values),
                "rainfall_magnitude": (("time", "y", "x"), magnitude_common['rainfall_magnitude'].values),
                "probability_map": (("time", "y", "x"), occurrence_common['prob_map'].values)
            },
            coords={
                "time": common_times,
                "x": magnitude_ds.x.values,
                "y": magnitude_ds.y.values
            }
        )
        IM_ds.x.attrs = magnitude_ds.x.attrs.copy()
        IM_ds.y.attrs = magnitude_ds.y.attrs.copy()
        IM_ds.time.attrs = magnitude_ds.time.attrs.copy()
        IM_ds.IM_product.attrs = {
            "units": "mm",
            "long_name": "Expected rainfall (Probability × Magnitude)",
            "description": "Product of occurrence probability map and rainfall magnitude map"
        }
        IM_ds.rainfall_magnitude.attrs = magnitude_ds.rainfall_magnitude.attrs.copy()
        IM_ds.probability_map.attrs = {
            "units": "probability",
            "long_name": "Rainfall occurrence probability",
            "description": "Probability of rainfall occurrence from previous stage"
        }
        save_dir = os.path.join(base_path, f'{workflow_type}_enhanced12')
...
def plot_kriging_comparison(
    ...
    ds_edk: xr.Dataset,
    shapefile_path: str,
    title_ok: str = "Ordinary Kriging (OK)",
    title_edk: str = "External Drift Kriging (EDK)"
):
    daily_df['day'] = pd.to_datetime(daily_df['day']).dt.date
    day_data = daily_df[daily_df['day'] == target_date]
    if day_data.empty:
        warnings.warn(f"No ground truth data found for {target_date}. Cannot generate plot.")
        return
    wet_stations = day_data[day_data['Rain'] > 0]
    dry_stations = day_data[day_data['Rain'] == 0]
    target_date_str = target_date.strftime('%Y-%m-%d')
    try:
        ok_grid = ds_ok['rainfall_magnitude'].sel(time=target_date_str)
        edk_grid = ds_edk['rainfall_magnitude'].sel(time=target_date_str)
    except KeyError:
        warnings.warn(f"No kriging data found for {target_date} in one of the datasets. Cannot generate plot.")
        return
    except Exception as e:
        warnings.warn(f"Error selecting date {target_date_str}: {e}. Check if date exists.")
        return
    try:
        sicily_border = gpd.read_file(shapefile_path).to_crs(epsg=32633)
    except Exception as e:
        warnings.warn(f"Could not load shapefile: {e}. Plotting without borders.")
        sicily_border = None
    ...
    axes = axes.flatten()
    fig.suptitle(f"Kriging Comparison for {target_date_str}", fontsize=22, y=0.96)
    max_rain = np.nanmax([
        wet_stations['Rain'].max() if not wet_stations.empty else 0,
        ok_grid.max(),
        edk_grid.max()
    ])
    if max_rain == 0 or not np.isfinite(max_rain):
        max_rain = 1
    cmap = 'RdYlBu' 
    norm = mcolors.Normalize(vmin=0, vmax=max_rain)
    ax = axes[0]
    ax.set_title("Ground Truth: Binary Occurrence", fontsize=14)
    if sicily_border is not None:
        sicily_border.plot(ax=ax, facecolor='lightgray', edgecolor='black', alpha=0.6)
    gdf_wet = gpd.GeoDataFrame(
        wet_stations, 
        geometry=gpd.points_from_xy(wet_stations.Longitude, wet_stations.Latitude), 
        crs="EPSG:32633"
    )
    gdf_dry = gpd.GeoDataFrame(
        dry_stations, 
        geometry=gpd.points_from_xy(dry_stations.Longitude, dry_stations.Latitude), 
        crs="EPSG:32633"
    )
    gdf_dry.plot(ax=ax, color='red', markersize=30, label='Dry (0 mm)', alpha=0.8, zorder=3)
    gdf_wet.plot(ax=ax, color='blue', markersize=40, edgecolor='black', linewidth=0.5, label='Wet (> 0 mm)', zorder=4)
    ax.legend(loc='upper right')
    ax.set_ylabel("Y-coordinate (meters)", fontsize=12)
    ax = axes[1]
    ax.set_title("Ground Truth: Rainfall Magnitude (mm)", fontsize=14)
    if sicily_border is not None:
        sicily_border.plot(ax=ax, facecolor='lightgray', edgecolor='black', alpha=0.6)
    gdf_wet.plot(
        ax=ax,
    ...
    ax = axes[2]
    ax.set_title(f"Interpolation: {title_edk}", fontsize=14)
    ax.pcolormesh(edk_grid.x, edk_grid.y, edk_grid, 
                  cmap=cmap, norm=norm, shading='auto')
    if sicily_border is not None:
        sicily_border.plot(ax=ax, facecolor='none', edgecolor='white', linewidth=1.5, zorder=2)
    ax.set_xlabel("X-coordinate (meters)", fontsize=12)
    ax.set_ylabel("Y-coordinate (meters)", fontsize=12)
    ax = axes[3]
    ax.set_title(f"Interpolation: {title_ok}", fontsize=14)
    ax.pcolormesh(ok_grid.x, ok_grid.y, ok_grid, 
                  cmap=cmap, norm=norm, shading='auto')
    if sicily_border is not None:
        sicily_border.plot(ax=ax, facecolor='none', edgecolor='white', linewidth=1.5, zorder=2)
    ax.set_xlabel("X-coordinate (meters)", fontsize=12)
    fig.subplots_adjust(right=0.85)
    pcm = axes[3].get_children()[0] 
    ...
    for ax in axes:
        ...
        if sicily_border is not None:
             ax.set_xlim(sicily_border.total_bounds[0], sicily_border.total_bounds[2])
             ax.set_ylim(sicily_border.total_bounds[1], sicily_border.total_bounds[3])
    plt.show()
...
if __name__ == '__main__':
    ...
    IM_ok = create_IM_product_from_file("OK", occurrence_ds)
    print("\n" + "="*50)
...
if __name__ == '__main__':
    ...
    ds_new_enhanced_model = xr.open_dataset(ok_enhanced_path,engine='netcdf4')
    print("Data loaded. Generating plot...")
    plot_kriging_comparison(
        target_date=date(2024,10,8),
        daily_df=daily_df,
        ds_ok=ds_old_model,
        ds_edk=ds_new_enhanced_model,
        shapefile_path=shapefile_path,
        title_ok="Old Model ", 
        title_edk="New Enhanced Model" 
    )
