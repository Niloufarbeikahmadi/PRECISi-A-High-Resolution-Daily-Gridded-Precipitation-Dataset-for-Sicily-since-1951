### code for Phase II Historical Reconstruction: magnitude modeling (method I2-IIA) ###
...
@dataclass
class Config:
    base_dir: Path = Path("outputs/reconstruction_I2")
...
def conditional_kriging_magnitude_ok(
    ...
    wet_mask = (binary_map == 1) & land_mask
    ...
    try:
        ...
        z_pred_trans, z_var_trans = OK.execute('masked', xpoints=grid_x, ypoints=grid_y, mask=~wet_mask)
        ...
        if np.any(valid_mask):
            ...
            result_grid[wet_mask] = temp_pred[wet_mask]
            variance_grid[wet_mask] = temp_var[wet_mask]
        return result_grid, variance_grid
    except Exception as e:
...
class OutputHandler:
    ...
    def save_monthly_data(
        ...
        self.logger.info(f"Saved monthly files for {year_month}: {magnitude_file.name}, {uncertainty_file.name}")
        return magnitude_file, uncertainty_file
...
class TransformerManager:
    ...
    def create_and_save_transformer(self, category: str, data: pd.DataFrame) -> Optional[QuantileTransformer]:
        ...
        if len(wet_data) < 10:
            ...
            dummy_data = np.random.randn(100).reshape(-1, 1)
...
class HistoricalMagnitudeReconstructionPipeline:
    ...
    def process_day(self, day: date, binary_map: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        ...
        return magnitude_map, uncertainty_map
    ...
    def process_monthly(self):
        ...
        for month_key in sorted(monthly_groups.keys()):
            ...
            binary_ds = self.data_loader.load_occurrence_binary_maps(month_key)
            if binary_ds is None:
                self.logger.error(f"Cannot load binary maps for {month_key}. Skipping month.")
                continue
            for day in tqdm(month_dates, desc=f"Processing {month_key}"):
                ...
                try:
                    ...
                    binary_map = binary_ds['binary_map'].sel(time=day_str).values
                    magnitude_map, uncertainty_map = self.process_day(day, binary_map)
                    if magnitude_map is not None and uncertainty_map is not None:
    ...
    def run(self):
        self.logger.info("=" * 80)
        self.logger.info("HISTORICAL MAGNITUDE RECONSTRUCTION PIPELINE (MONTHLY SAVING)")
        self.logger.info("=" * 80)
        try:
            self.load_all_data()
